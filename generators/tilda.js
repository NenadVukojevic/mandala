...
function generateShapes(centerX, centerY, outer, size) {
    ...
    for (let i = 0; i < pearlCount; i++) {
        ...
        const coordinate = getCoordinates(centerX, centerY, bRadius, axAngle);
        var startAngle =  axAngle;
        var endAngle = axAngle + 95;
        shapesArray.push({type:'arc', x: coordinate.x, y: coordinate.y, radius: elementSize/2, startAngle: deg2rad(startAngle), endAngle: deg2rad(endAngle) },);
        const coordinateB = getCoordinates(centerX, centerY, outer, axAngle)
        shapesArray.push({type:'arc', x: coordinateB.x, y: coordinateB.y, radius: elementSize/2, startAngle: deg2rad(startAngle+180), endAngle: deg2rad(endAngle+180) },);
      
        /*
        for (let j = axAngle - 75; j<= axAngle + 75; j+=15)
        {
            const ofCoordinate = getCoordinates(coordinate.x, coordinate.y, elementSize, j);
            shapesArray.push({type:'line', start:coordinate, end:ofCoordinate});
        }
            */
    }
